...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderList(rows, { showItemLabel = false } = {}) {
    if (!rows.length) return renderEmpty("No gift givers yet.");
    ...
    listEl.innerHTML = rows.map(r => {
      ...
      const receiptBtn = receipt
        ? `<span class="donor-receipt" data-bill="${encodeURIComponent(receipt)}">gift receipt</span>`
        : "";
  ...
  function renderAll() {
    ...
    if (!flat.length) return renderEmpty("No gift givers yet.");

    // group
    const grouped = groupDonors(flat).sort((a,b) => money(b.amount) - money(a.amount));

    const total = grouped.reduce((s,x)=> s + money(x.amount), 0);
    summaryEl.textContent = `Total gift givers: ${grouped.length} — Total gifted: ${total} EGP`;
  ...
  function renderItem() {
    ...
    summaryEl.textContent = `Item: ${title} — Gift givers: ${donors.length} — Total: ${total} EGP`;
