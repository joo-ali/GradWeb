...
function safeTitle(title) {
  const raw = String(title || "item").trim();
  ...
  if (!slug) slug = `item_${hashStorageSegment(raw)}`;
...
function renderWishlist(user) {
  ...
  const safeImage = escapeHtml(item?.image || "images/placeholder.webp");
  ...
  <div class="wishlist-card ${isFunded ? "funded" : ""}">
    ...
    <div class="item-info">
      ...
      ${isFunded ? `<div class="funded-badge">Fully Gifted 🎉</div>` : ``}
    </div>
  </div>
  `;
...
This item already has gifts/gift givers. The item will be removed from Strapi and its Supabase images/receipts folder will be deleted too.`
